...
exports.createReceipe = async (req, res) => {
  ...
  console.log(req.body);
    ...
    const newRecipe = new Recipe({
      ...
      ingredients: ingredientInputs?.map(({ name, description }) => ({ name, description: description })),
      steps: stepInputs || [],
      overview: overviewInputs?.map(({ name, description }) => ({ name, description: description })),
